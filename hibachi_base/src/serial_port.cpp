...
hibachi_base::SerialPort::SerialPort(const std::string &port)
    : _serialPort(port),
      _txBuffer(NULL),
      _txBufferSize(0),
      _txBufferLength(0),
...
hibachi_base::SerialPort::~SerialPort() {
  closeConnection();
  setTxBufferSize(0);
  setRxBufferSize(0);
  setMaxFrameLength(0);
...
void hibachi_base::SerialPort::setPort(const std::string &port) {
  if (port != _serialPort) {
    closeConnection();
    _serialPort = port;
  }
...
void hibachi_base::SerialPort::setTxBufferSize(size_t size) {
...
void hibachi_base::SerialPort::setRxBufferSize(size_t size) {
...
void hibachi_base::SerialPort::setMaxFrameLength(size_t length) {
...
bool hibachi_base::SerialPort::openConnection() {
  try {
    if (_serial.isOpen()) {
      return true;
    }

    auto currentTime = boost::chrono::system_clock::now();
    boost::chrono::duration<double> elapsedConnectTime =
        currentTime - _lastConnectTime;
...
void hibachi_base::SerialPort::closeConnection() {
  try {
    if (_serial.isOpen()) {
      _serial.close();
    }
...
void hibachi_base::SerialPort::sendMessage(Message *message) {
  if (message == NULL) {
    return;
  }
...
hibachi_base::Message *hibachi_base::SerialPort::receiveMessage() {
  read();
...
hibachi_base::Message *hibachi_base::SerialPort::waitMessage(double timeout) {
...
hibachi_base::Message *hibachi_base::SerialPort::waitMessage(uint16_t messageType,
                                                             double timeout) {
  auto start_time = boost::chrono::system_clock::now();
  while (true) {
...
void hibachi_base::SerialPort::read() {
  try {
    if (openConnection()) {
...
void hibachi_base::SerialPort::writeFrame(const uint8_t *data, size_t length) {
...
void hibachi_base::SerialPort::encodeByte(uint8_t data, bool escape) {
  if (escape && ((data == AMPRU_HDLC_ESCAPE_FLAG) ||
                 (data == AMPRU_HDLC_FRAME_BOUNDRY_FLAG))) {
...
void hibachi_base::SerialPort::decodeByte(uint8_t data) {
  if (data == AMPRU_HDLC_FRAME_BOUNDRY_FLAG) {
    if (_rxEscapeByte) {
      _rxEscapeByte = false;
    } else if (_frameBufferLength >= 2 &&
               _frameChecksum == ((_frameBuffer[_frameBufferLength - 1] << 8) |
                                  _frameBuffer[_frameBufferLength - 2])) {
  ...
  if (_frameBufferLength < (_frameBufferSize - 1)) {
    ...
    if (_frameBufferLength >= 2) {
      _frameChecksum =
          crc_ccitt_update(_frameChecksum, _frameBuffer[_frameBufferLength - 2]);
    }
...
uint16_t hibachi_base::SerialPort::crc_ccitt_update(uint16_t crc, uint8_t data) {
